    ...
    <div className='reviews-sec bg-yellow-300 py-20'>
      <div className="mx-[30px] lg:mx-[90px] relative">
        <div className='flex justify-center lg:text-5xl  text-orange-800'>
          <Image layout="intrinsic" priority src={head_style_left} alt='icon ' className='hidden lg:block'/>
            <p >Our Students <span className='bg-orange-800 text-white p-1 lg:p-3'>Placed</span></p> 
          <Image layout="intrinsic" priority src={head_style_right} alt='icon' className='hidden lg:block'/>
        </div>
      ...
      {list.map((item)=>(
        <CarouselItem className="xl:basis-1/3 lg:basis-1/2 pt-36 relative" key={item.id}>
          <div className='flex justify-center '>
             <Image layout="intrinsic" priority src={item.student_images} alt={item.name} className='w-28 student-image absolute top-10'/>
          </div>
        <Card className="pt-10 rounded-2xl">
         <CardHeader>
          ...
          <CardFooter className="flex justify-between pr-0">
             <div>
               <Image layout="intrinsic" priority src={item.image} alt={item.name} className='w-36 h-10'/> 
             </div>
